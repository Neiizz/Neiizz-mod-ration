...
module.exports = {
    name: "rolelogs", 
    description: "Activer ou désactiver les logs des roles", 
    usage: "-rolelogs <on/off> [salon]", 
    categorie: "perm3",
    /**
     * 
     * @param {bot} client 
    ...
    run: async(client, message, args, lang, commandName) => {
        ...
        .setDescription(`**Ce salon est déjà attribué aux logs des roles**`)
        .setColor(color || client.config.color)

        const embed4 = new Discord.MessageEmbed()
        .setDescription(`**Les logs des roles sont déjà désactivés**`)
        .setColor(color || client.config.color)

        const embed5 = new Discord.MessageEmbed()
        .setDescription(`Les logs des roles sont désormais **désactivés**`)
        .setColor(color || client.config.color)
        ...
        if(args[0] === "on"){
            ...
            if(client.db["logs"].get(`rolelogs_${message.guild.id}`) === channel.id) return message.channel.send({ embeds: [embed2] }); 

            client.db["logs"].set(`rolelogs_${message.guild.id}`, channel.id).save();

            const embed3 = new Discord.MessageEmbed()
            .setDescription(`Les logs des roles sont désormais **activés** dans <#${channel.id}>`)
            .setColor(color || client.config.color)

            message.channel.send({ embeds: [embed3] }); 
        }
        if(args[0] === "off"){
            if(client.db["logs"].get(`rolelogs_${message.guild.id}`) === null || client.db["logs"].get(`rolelogs_${message.guild.id}`) === undefined) return message.channel.send({ embeds: [embed4] });  

            client.db["logs"].delete(`rolelogs_${message.guild.id}`); 
