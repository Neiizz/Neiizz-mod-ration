...
module.exports = {
    ...
    run: async(client, message, args, lang, commandName) => {
        ...
        .setDescription(`**Ce salon est déjà attribué aux logs des modérations**`)
        .setColor(color || client.config.color)

        const embed4 = new Discord.MessageEmbed()
        .setDescription(`**Les logs des modérations sont déjà désactivés**`)
        .setColor(color || client.config.color)

        const embed5 = new Discord.MessageEmbed()
        .setDescription(`Les logs des modérations sont désormais **désactivés**`)
        .setColor(color || client.config.color)
        ...
        if(args[0] === "on"){
            ...
            if(client.db["logs"].get(`modlogs_${message.guild.id}`) === channel.id) return message.channel.send({ embeds: [embed2] }); 

            client.db["logs"].set(`modlogs_${message.guild.id}`, channel.id).save();
            
            const embed3 = new Discord.MessageEmbed()
            .setDescription(`Les logs des modérations sont désormais **activés** dans <#${channel.id}>`)
            .setColor(color || client.config.color)

            message.channel.send({ embeds: [embed3] }); 
        }
        if(args[0] === "off"){
            if(client.db["logs"].get(`modlogs_${message.guild.id}`) === null || client.db["logs"].get(`modlogs_${message.guild.id}`) === undefined) return message.channel.send({ embeds: [embed4] }); 
