...
module.exports = {
    name: "channellogs", 
    aliases: ["salonlogs", "chlogs"], 
    description: "Voir les logs des salons", 
    usage: "-chlogs", 
    categorie: "perm3",
    /**
     * 
    ...
    run: async(client, message, args, lang, commandName) => {
        ...
        .setDescription(`**Ce salon est déjà attribué aux logs des salons**`)
        .setColor(color || client.config.color)

        const embed4 = new Discord.MessageEmbed()
        .setDescription(`**Les logs des salons sont déjà désactivés**`)
        .setColor(color || client.config.color)

        const embed5 = new Discord.MessageEmbed()
        .setDescription(`Les logs des salons sont désormais **désactivés**`)
        .setColor(color || client.config.color)
        ...
        if(args[0] === "on"){
            ...
            if(client.db["logs"].get(`chlogs_${message.guild.id}`) === channel.id) return message.channel.send({ embeds: [embed2] }); 

            client.db["logs"].set(`chlogs_${message.guild.id}`, channel.id).save();

            const embed3 = new Discord.MessageEmbed()
            .setDescription(`Les logs des salons sont désormais **activés** dans <#${channel.id}>`)
            .setColor(color || client.config.color)

            message.channel.send({ embeds: [embed3] }); 
        }
        if(args[0] === "off"){
            if(client.db["logs"].get(`chlogs_${message.guild.id}`) === null || client.db["logs"].get(`chlogs_${message.guild.id}`) === undefined) return message.channel.send({ embeds: [embed4] });
