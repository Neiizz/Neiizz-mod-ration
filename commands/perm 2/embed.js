...
module.exports = {
    ...
    run: async(client, message, args, commandName) => {
        ...
        if(pass === 'no') return message.reply(`\`⛔️\` *Erreur* **Tu doit être dans la \`permissions 2\` pour utilisé cette commande **`)
        ...
        const functions = {
          ...
          const reactionsArray = [
              ...
              {emoji: '✨', description: "Ajouter un field", fnt: 'field', label: "Champs"},
              {emoji: '📘', description: 'Ajouter un footer', fnt: "footer_text", label: ""},
              {emoji: '📙', description: "Configurer l'image du footer", fnt: "footer_img", label: ""},
          ...
          reactionsArray.forEach(async(reaction) => {
              if (i < 3) {
                  i++;
                  menuEmbed.addField(reaction.emoji, `> ${reaction.description}`, true);
              } else {
                  i = 0;
                  menuEmbed.addField(reaction.emoji, `> ${reaction.description}`, false);
              };
          });
              ...
              const menu = menuiserie;
                  ...
                  const fnts = {
                      ...
                      description: async() => {
                          const collector = message.channel.createMessageCollector({ filter: (msg) => msg.author.id === message.author.id,  time: 120000});
                          var toSupp = [];
                          message.channel.send({embeds: [new Discord.MessageEmbed().setFooter({ text: "Tapez cancel pour annuler et vous avez deux minutes" })
                          .setTimestamp()
                          .setColor('WHITE')
                      ...
                      footer_img: async() => {
                          ...
                          .setTimestamp()
                          .setColor('WHITE')
                          .setDescription(`❓ | Quel est le lien de l'image du footer ?`)
                          .setTitle("Image du footer")]}
                          ).then(async(x) => {toSupp.push(x)});
                          ...
                          collector.on('collect', async(msg) => {
                              ...
                              if (msg.content.toLowerCase() === 'cancel') return collector.stop() & message.channel.send({embeds: [new Discord.MessageEmbed()
                                  .setTitle("Annulé")
                                  .setTimestamp()
                                  .setDescription(`✅ | Annulation de la modification de l'image du footer`)
                                  .setColor("WHITE")]}
                              ).then(x => {setTimeout(() => {functions.del(x)}, 1000)});
                      ...
                      footer_text: async() => {
                          ...
                          .setTimestamp()
                          .setTitle("Footer")
                          .setDescription(`❓ | Quel est le footer?`)
                          .setColor('WHITE')]}
                          ).then(async(x) => {toSupp.push(x)});
                          ...
                          collector.on('collect', async(msg) => {
                              ...
                              if (msg.content.toLowerCase() === 'cancel') return collector.stop() & message.channel.send({
                                  ...
                                  .setTimestamp()
                                  .setColor("WHITE")
                                  .setDescription(`✅ | Annulation de la modification du footer`)
                                  ]
                              }
                              ).then(x => {setTimeout(() => {functions.del(x)}, 1000)});
                      ...
                      field: async() => {
                          ...
                          message.channel.send({embeds: [new Discord.MessageEmbed()
                              .setFooter({ text: "Tapez cancel pour annuler et vous avez deux minutes" })
                              .setTimestamp()
                              .setTitle("Field")
                              .setDescription(`ℹ | Donnez trois valeurs : le titre, le texte et si il est "inline" (oui ou non) séparés par \`--\`.\nExemples: le titre du field -- Ceci est la description -- non\nLien -- cliquez ici -- oui`)
                              .setColor('WHITE')]}
                          ).then(async(x) => {toSupp.push(x)});
                          ...
                          collector.on('collect', async(msg) => {
                              ...
                              if (msg.content.toLowerCase() === 'cancel') return collector.stop() & message.channel.send({embeds: [new Discord.MessageEmbed()
                                  .setTitle("Annulé")
                                  .setTimestamp()
                                  .setDescription(`:white_check_mark: | Annulation de la création de field`)
                                  .setColor("WHITE")]}
                              ).then(x => {setTimeout(() => {functions.del(x)}, 1000)});
                      ...
                      url: async() => {
                          ...
                          message.channel.send({embeds: [
                              new Discord.MessageEmbed().setFooter({ text: "Tapez cancel pour annuler et vous avez deux minutes" })
                              .setTimestamp()
                              .setTitle("URL")
                  ...
                  bigCollector.on('collect', async(i) => {
                      ...
                      i.reply({ content: `Suivez les étapes suivantes`, ephemeral: false });
                      setTimeout(() => {
                          i.deleteReply().catch(() => {});
                      }, 2000)
                  });
